...
const packages = [
  {
    id: 'intimate',
    name: 'Камерный',
    tagline: 'Подходит для росписи, прогулки или уютной съёмки только для вас двоих.',
    features: [
      'до 4 часов съёмки',
  ...
  return (
    <div className={styles.page}>
      ...
      >
        ...
        <motion.p className={styles.lead} variants={heroItem(prefersReducedMotion)}>
          Каждая свадьба — особенная, поэтому формат съёмки подбирается индивидуально: от камерной
          прогулки до полноценного свадебного дня.
        </motion.p>
      </motion.header>

      <section className={styles.packages} aria-label="Пакеты съёмки">
        <motion.div className={styles.packageGrid} {...cardReveal(prefersReducedMotion, 0)}>
          {packages.map((plan) => (
            <article
              key={plan.id}
              className={`${styles.card} ${plan.featured ? styles.cardFeatured : ''}`}
            >
              {plan.featured ? <span className={styles.badge}>Полный день</span> : null}
              ...
              <div className={styles.priceBlock}>
                ...
                {plan.extra ? <p className={styles.priceExtra}>{plan.extra}</p> : null}
              </div>
            </article>
          ))}
        </motion.div>
      ...
      >
        <div className={styles.customInner}>
          ...
          <p className={styles.customText}>
            Если вы планируете венчание, крещение, love story, контент-съёмку, съёмку за пределами
            города или хотите создать полностью индивидуальный формат — условия обсуждаются отдельно.
          </p>
          <p className={styles.customText}>
            Для меня важно не просто предложить пакет, а подобрать съёмку именно под вашу историю,
            атмосферу дня и ваши пожелания, чтобы видео получилось по-настоящему живым, красивым и
            вашим.
          </p>
        </div>
      </motion.section>
